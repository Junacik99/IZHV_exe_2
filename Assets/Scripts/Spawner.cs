...
public class Spawner : MonoBehaviour
{
    ...
    /// <summary>
    /// Size of the small spawned obstacles.
    /// </summary>
    public float smallSize = 1.0f;

    /// <summary>
    /// Size of the big spawned obstacles, that cannot be jumped over.
    /// </summary>
    public float bigSize = 1.0f;
    ...
    void Update()
    {
        ...
        { // Check if we should spawn.
            ...
            { // Spawn at most one obstacle per frame.
                ...
                float spawn2 = Random.Range(0f, 1.0f);
                if (spawn2 > 0.75f)
                    Spawn2Obstacles();
                else
                    SpawnObstacle();
    ...
    public void SpawnObstacle()
    {
        ...
        obstacle.transform.position += (Vector3)(spawnDown ? 
            spawnOffset + (1.0f - smallSize) / 2.0f : 
            -spawnOffset - (1.0f - smallSize) / 2.0f
        );
    ...
    /// <summary>
    /// Spawn 2 obstacles on both sides
    /// </summary>
    public void Spawn2Obstacles()
    {
        // Spawn the obstacle.
        var obstacleTop = Instantiate(obstaclePrefab, transform);
        var obstacleBot = Instantiate(obstaclePrefab, transform);


        // Move it to the target location.
        obstacleTop.transform.position += (Vector3)(spawnOffset + (1.0f - smallSize) / 2.0f);
        obstacleBot.transform.position += (Vector3)(-spawnOffset - (1.0f - smallSize) / 2.0f);

        obstacleTop.transform.localScale = new Vector3(smallSize, smallSize, smallSize);
        obstacleBot.transform.localScale = new Vector3(smallSize, smallSize, smallSize);

        // Move the obstacle into the correct layer.
        obstacleTop.layer = LayerMask.NameToLayer(spawnLayer);
        obstacleBot.layer = LayerMask.NameToLayer(spawnLayer);
